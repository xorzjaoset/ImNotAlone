		...
		<div class="box ui-sortable-handle"
			...
			<div id="borderedTable" class="body collapse in">
				<table class="table table-bordered responsive-table">
					...
					<tbody>
						<c:forEach var="black" items="${blackwait}">
							<tr>
								<td>${black.no}</td>
								<td>${black.id}</td>
								<td>${black.reg_dt}</td>
								<td>${black.reason}</td>
								<td>
									<input type="submit" value="블랙" class="form-control" >
								</td>
								<td><a href="/ImNotAlone/share/police112?id=${black.id}" onclick="window.open(this.href, '_blank', 'width=350px, height=260px, toolbars=no, scrollbars=no'); return false;">신고하기</a>
							</tr>
						</c:forEach>
					</tbody>
				</table>
